...
const TestCanvas = () => {
  ...
  useEffect(() => {
    const doParallax = (cursorX: number, cursorY: number) => {
      ...
      if (layer1 && layer2) {
        layer1.style.transform = `translate3d(${pointX / 40}px, ${pointY / 40}px, 0px)`;
        layer2.style.transform = `translate3d(${pointX / 20}px, ${pointY / 30}px, 0px)`;
      }
    };
  ...
  return (
    <ContainerTest>
      ...
      <div style={{ width: '2000px', height: '100%', backgroundColor: 'red' }} />
    </ContainerTest>
  );
};
